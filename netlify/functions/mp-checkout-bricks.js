...
exports.handler = async (event) => {
  console.log("[mp-checkout] INVOCADO - Method: " + event.httpMethod + " | Time: " + new Date().toISOString());
    try {
        ...
        // Bricks usa o preference_id (não init_point). init_point só existiria para Checkout Pro.
        if (!r.ok || !data.id) {
            console.error('Erro ao criar preferência MP (Bricks):', data);
            return { statusCode: 400, body: JSON.stringify({ error: 'Falha ao criar pagamento no Mercado Pago', details: data }) };
        }

        return { statusCode: 200, body: JSON.stringify({ preference_id: data.id, order_id: orderId, applied_coupon: appliedCoupon, final_price: price, discount: discount }) };
