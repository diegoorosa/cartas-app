// ARQUIVO: netlify/functions/mp-checkout2.js
// TESTE: usa checkout_intents2 com colunas de cupom
...
async function logConsent(supabase, orderId, payload, event, slug) {
    try {
        if (!payload || !payload.accepted_terms) return;
        ...
        if (error) console.error('Erro ao salvar consentimento:', error);
    } catch (err) {
...
exports.handler = async (event) => {
  console.log("[mp-checkout2] INVOCADO - Method: " + event.httpMethod + " | Time: " + new Date().toISOString());
    try {
        ...
        if (coupon && COUPONS[coupon]) {
            if (coupon === 'VOLTA10' && lead_created_at) {
                ...
                if (hoursDiff > 24) {
                    console.log(`[mp-checkout2] Cupom VOLTA10 expirado (${hoursDiff.toFixed(1)}h > 24h), ignorando desconto`);
                } else {
        ...
        console.log('[mp-checkout2] order_id gerado:', orderId);

        // Salva na checkout_intents2 (teste, com colunas de cupom)
        try {
            await supabase.from('checkout_intents2').insert({
                order_id: orderId,
                slug,
            ...
            console.log('[mp-checkout2] checkout_intents2 salvo com sucesso para order_id:', orderId);
        } catch (e) {
            console.error('[mp-checkout2] ERRO ao salvar intent no Supabase:', e);
        }
        ...
        const pref = {
            ...
            notification_url: `${BASE_URL}/.netlify/functions/mp-webhook2`,
        ...
        console.error('Erro fatal no checkout2:', e);
