    ...
    for (let i = 0; i < tries; i++) {
        try {
            ...
            if (retryable && i < tries - 1) {
                await sleep(500 * Math.pow(2, i) + Math.floor(Math.random() * 300));
                continue;
            }
            throw err;
        }
...
const SYSTEM_CARTA =
    'Você gera cartas e requerimentos formais no padrão brasileiro. Responda SOMENTE em JSON válido no formato: {"titulo":"","saudacao":"","corpo_paragrafos":["..."],"fechamento":"","check_list_anexos":["..."],"observacoes_legais":""}. Tom formal, claro e respeitoso, português do Brasil. Produza 3 a 4 parágrafos de 60 a 90 palavras cada, sem placeholders. Estruture: 1) identificação do remetente e pedido de cancelamento imediato, 2) cessação de cobranças futuras e exclusão de débitos automáticos, 3) confirmação por escrito com protocolo e data, 4) estorno se houver cobrança posterior indevida e prazo de resposta. Use formulações como "reservando-me o direito de adotar as providências administrativas cabíveis". Inclua check_list_anexos pertinente. Em observacoes_legais, mencione genericamente o Código de Defesa do Consumidor (Lei 8.078/90), sem aconselhamento jurídico.';

const SYSTEM_VIAGEM =
    'Você gera AUTORIZAÇÃO DE VIAGEM PARA MENOR no padrão brasileiro. Responda SOMENTE em JSON válido no formato: {"titulo":"","saudacao":"","corpo_paragrafos":["..."],"fechamento":"","check_list_anexos":["..."],"observacoes_legais":""}. Tom formal, claro, PT-BR, sem placeholders. Produza 3 a 5 parágrafos contendo: 1) identificação do menor (nome, data de nascimento, documento) e dos responsáveis que assinam (nomes, CPFs, documentos e parentesco), 2) tipo de viagem (nacional/internacional), destino e período (datas), 3) se houver acompanhante, identificar nome, documento e parentesco; se sem acompanhante, explicitar, 4) autorização expressa restrita ao período/destino informados, 5) local de assinatura. Inclua linhas de assinatura no fechamento. Inclua check_list_anexos: cópias dos documentos do menor e dos responsáveis, comprovante de parentesco quando aplicável, e duas vias assinadas. Em observacoes_legais, cite genericamente ECA e exigências de autoridades/companhias, sem aconselhamento jurídico.';
...
exports.handler = async (event) => {
    try {
        ...
        const isViagem = String(payload.tipo || '') === 'autorizacao_viagem';
