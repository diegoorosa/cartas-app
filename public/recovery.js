(function () {
    ...
    function downloadDocFromHtml(html, filename) {
        var full = '<html><head><meta charset="utf-8"><style>body{background:#fff;color:#000;font:12pt "Times New Roman",Times,Georgia,serif;line-height:1.6;margin:0;padding:18mm;box-sizing:border-box}p{margin:0 0 12pt 0;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere}</style></head><body>' + html + '</body></html>';
        var blob = new Blob(['\ufeff', full], { type: 'application/msword' });
        var url = URL.createObjectURL(blob);
        var a = document.createElement('a'); a.href = url; a.download = (filename || 'documento') + '.doc'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    }
    ...
    function downloadPdfFromHtml(html, filename) {
        ensureHtml2pdf().then(function () {
            ...
            host.innerHTML = '<div class="pdf-page">' + html + '</div><style>.pdf-page{width:210mm;min-height:297mm;padding:18mm;box-sizing:border-box;background:#fff;color:#000;font:12pt "Times New Roman",Times,Georgia,serif;line-height:1.6;word-break:break-word;overflow-wrap:anywhere} .pdf-page p{margin:0 0 12pt 0;white-space:pre-wrap} .pdf-page *{color:#000 !important;background:transparent !important}</style>';
            document.body.appendChild(host);
            var node = host.querySelector('.pdf-page');
            var waitFonts = (document.fonts && document.fonts.ready) ? document.fonts.ready : Promise.resolve();
            waitFonts.then(function () {
                var opt = { margin: 0, filename: (filename || 'documento') + '.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, backgroundColor: '#ffffff' }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: ['css', 'legacy'] } };
                html2pdf().set(opt).from(node).save().then(function () { host.remove(); });
            });
        });
    }
    ...
    function offerRecover() {
        var state = loadGeneratedDoc(); if (!state) return; injectStyles();
        var n = (state && state.meta && state.meta.filename) || 'documento';
        var t = (state && state.meta && state.meta.type) || '';
        var id = (state && state.meta && state.meta.orderId) || '';
        var modal = document.createElement('div');
        modal.className = 'lm-modal';
        modal.innerHTML = '<div class="lm-card"><h3 style="margin:0 0 8px">Documento encontrado</h3><p>Temos um documento ' + (t ? ('(' + t + ') ') : '') + 'gerado recentemente' + (id ? (' • Pedido: ' + id) : '') + '. Deseja recuperar?</p><div class="lm-actions"><button class="btn btn-primary" id="lm-rec-doc">Baixar DOC</button><button class="btn" id="lm-rec-pdf">Baixar PDF</button><a class="btn btn-green" id="lm-rec-wa" target="_blank" rel="noopener">WhatsApp</a><button class="btn btn-outline" id="lm-rec-clear">Descartar</button></div></div>';
        document.body.appendChild(modal);
        document.getElementById('lm-rec-wa').href = 'https://wa.me/' + WHATSAPP + '?text=' + encodeURIComponent('Oi! Preciso de ajuda com meu documento. Tipo: ' + t + ' | Pedido: ' + id);
        document.getElementById('lm-rec-doc').onclick = function () { downloadDocFromHtml(state.html, n); };
        document.getElementById('lm-rec-pdf').onclick = function () { downloadPdfFromHtml(state.html, n); };
        document.getElementById('lm-rec-clear').onclick = function () { clearGeneratedDoc(); modal.remove(); };
